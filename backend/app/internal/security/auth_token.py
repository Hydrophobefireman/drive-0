...
def regenerate_access_token(refresh: dict) -> dict:
    ...
    if check(integrity, current):
        return (
            issue_access_token(user_id, is_admin),
            issue_refresh_token(user_id, data.password_hash),
        )
    return None, None


def issue_access_token(user_id: str, is_admin: bool):
    return {
...
def authenticate(user: str, password: str):
    ...
    access_token = create_token(issue_access_token(user_data.id_, is_admin))
